...
# Run experiments
learning_rates = [item+abs(np.random.normal(0,item)) for item in np.logspace(-5,0,6)]
for ind, learning_rate in enumerate(learning_rates):
    ...
    with open(work_dir+os.sep+str(ind)+'_info.txt','w') as f:
        to_write = 'Learning rate: '+str(learning_rate)+\
            '\nDropout: '+str(dropout)+\
    ...
    if True:
        ...
        plt.savefig(work_dir+os.sep+str(ind)+'_accuracy.jpg')
        plt.close()
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'validation'], loc='upper left')
        plt.savefig(work_dir+os.sep+str(ind)+'_loss.jpg')
